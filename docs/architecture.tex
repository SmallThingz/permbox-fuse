...
\title{permfuse: a Policy Layer over Linux OverlayFS}
\author{permbox-fuse3}
\date{\today}
...
\begin{abstract}
permfuse is a library that presents a filesystem through a dynamic path
...
\node[box,draw=policy,above=of overlay] (fuse) {permfuse policy filesystem};
...
permfuse then applies its four-state path policy. OverlayFS finally performs
...
permfuse does not attempt to make it safe. Kernel OverlayFS and FUSE support,
